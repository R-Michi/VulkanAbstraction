...
void vka::Texture::destroy_handles() const noexcept
{
...
void vka::Texture::internal_create(const VkPhysicalDeviceMemoryProperties& properties, const TextureCreateInfo& create_info)
{
    // create image
    ...
    check_result(vkCreateSampler(this->m_device, &sampler_ci, nullptr, &this->m_sampler), SAMPLER_CREATE_FAILED);
...
void vka::Texture::create_view(const TextureViewCreateInfo& create_info)
{
    if (this->is_valid())
    {
        ...
        VkImageView view;
        check_result(vkCreateImageView(this->m_device, &ci, nullptr, &view), VIEW_CREATE_FAILED);
...
vka::Buffer vka::Texture::stage(const void* const* data, const VkPhysicalDeviceMemoryProperties& properties, uint32_t qfamidx, uint32_t layer_count, uint32_t level) const
{
    if (this->m_state != STATE_INVALID)
    {
        ...
        Buffer staging(this->m_device, properties, create_info);
