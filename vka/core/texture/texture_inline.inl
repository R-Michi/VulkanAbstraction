...
inline vka::Texture::Texture() noexcept :
...
inline void vka::Texture::destroy() noexcept
{
    this->destroy_handles();
...
inline void vka::Texture::change_layout_F2L(VkCommandBuffer cbo) noexcept
{
    ...
    // as we don't know the specific shader pipeline stages. But all commands include any shader
...
inline vka::Buffer vka::Texture::stage(const void* data, const VkPhysicalDeviceMemoryProperties& properties, uint32_t qfamidx, uint32_t level) const
{
    return this->stage(&data, properties, qfamidx, 1, level);
}
...
inline VkExtent3D vka::Texture::size() const noexcept
{
    return this->m_extent;
}

inline uint32_t vka::Texture::level_count() const noexcept
{
...
inline uint32_t vka::Texture::layer_count() const noexcept
{
    return this->m_layer_count;
}

inline VkImage vka::Texture::handle() const noexcept
{
    return this->m_image;
}

inline VkSampler vka::Texture::sampler() const noexcept
{
...
inline bool vka::Texture::is_valid() const noexcept
{
