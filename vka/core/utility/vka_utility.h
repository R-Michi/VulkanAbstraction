...
namespace vka::utility
{
    ...
    * 'pool'. The allocated command buffer is returned. If an error occured while allocating
    ...
    * for the submission. If an error occured while ending the command buffer recording or while
    * submitting the command buffer to the queue, a std::runtime_error exception is thrown
    ...
    * is specified. If an error occured while waiting on either the queue or the fence, a
