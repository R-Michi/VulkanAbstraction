...
inline vka::Buffer::Buffer(void) noexcept :
...
inline vka::Buffer::Buffer(VkDevice device, const VkPhysicalDeviceMemoryProperties& properties, const vka::BufferCreateInfo& create_info) :
    m_device(device),
    m_buffer(VK_NULL_HANDLE),
    m_memory(VK_NULL_HANDLE),
    m_size(0),
...
inline vka::Buffer::Buffer(vka::Buffer&& src) noexcept :
...
inline vka::Buffer& vka::Buffer::operator=(vka::Buffer&& src) noexcept
{
...
inline vka::Buffer::~Buffer(void)
{
    this->destroy_handles();
}

inline void vka::Buffer::create(VkDevice device, const VkPhysicalDeviceMemoryProperties& properties, const vka::BufferCreateInfo& create_info)
{
    if (!this->is_valid())
    {
...
inline void vka::Buffer::destroy(void) noexcept
{
    this->destroy_handles();
